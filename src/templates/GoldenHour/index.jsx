...
const COLORS = { sand: '#E8D5B7', terracotta: '#C4713E', brown: '#3D2B1F', ivory: '#FDFAF4' }

export default function GoldenHour({ wedding, guest, isPersonalized }) {
  ...
  const primaryColor = wedding?.primary_color || COLORS.terracotta
  ...
  useEffect(() => {
    ...
    gsap.fromTo(chars, { opacity: 0, y: 30 }, { opacity: 1, y: 0, stagger: 0.04, duration: 0.7, ease: 'power2.out', delay: 0.3 })
  }, [loaded])

  const splitName = (name) => name?.split('').map((ch, i) => <span key={i} className="char inline-block">{ch === ' ' ? '\u00A0' : ch}</span>)

  return (
    <div style={{ fontFamily: '"Nunito", sans-serif', background: COLORS.ivory, minHeight: '100vh' }}>
      ...
      {!loaded && <div className="h-screen" style={{ background: COLORS.ivory }} />}
      {loaded && (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
          <FloatingPetals template="GoldenHour" />
          ...
          <section id="hero" className="relative min-h-screen flex flex-col items-center justify-center px-4 pt-16 pb-12 overflow-hidden"
            style={{ background: `linear-gradient(160deg, ${COLORS.ivory} 30%, ${COLORS.sand} 100%)` }}>
            <div className="relative z-10 max-w-2xl w-full text-center">
              {isPersonalized && (
                <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.5 }} className="mb-6">
                  <p className="text-xs tracking-[0.4em] uppercase mb-1" style={{ color: primaryColor, fontFamily: 'Nunito' }}>Exclusive Invitation For</p>
                  <h1 className="text-2xl sm:text-3xl font-baskerville italic" style={{ color: COLORS.brown }}>{guest?.name}</h1>
                </motion.div>
              )}
              ...
              <div ref={namesRef}>
                <h2 className="text-4xl sm:text-6xl font-baskerville italic mb-2" style={{ color: COLORS.brown }}>
                  {splitName(wedding?.groom_name)}
                </h2>
                <p className="text-lg tracking-widest mb-2" style={{ color: primaryColor }}>& </p>
                <h2 className="text-4xl sm:text-6xl font-baskerville italic" style={{ color: primaryColor }}>
              ...
              {weddingDate && (
                <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.9 }}
                  ...
                  <div className="px-6 py-4 text-center" style={{ background: primaryColor + '15' }}>
                    <p className="text-5xl font-baskerville font-bold" style={{ color: primaryColor }}>{weddingDate.getDate()}</p>
                  </div>
                ...
                <motion.div ref={containerRef} initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 1 }}
                  ...
                  <motion.div style={{ rotateX: tx, rotateY: ty, transformStyle: 'preserve-3d' }} className="w-full h-full relative">
                    ...
                    <img src={wedding.cover_photo_cutout_url || wedding.cover_photo_url} alt="Couple" className="absolute inset-0 w-full h-full object-contain"
                      style={{ transform: 'translateZ(40px)', filter: 'drop-shadow(0 40px 80px rgba(0,0,0,0.35))' }} />
                  </motion.div>
                </motion.div>
              )}
          ...
          <footer className="py-12 px-4 text-center border-t" style={{ borderColor: primaryColor + '20', background: COLORS.ivory }}>
            <p className="font-baskerville italic text-2xl mb-1" style={{ color: primaryColor }}>{wedding?.groom_name} & {wedding?.bride_name}</p>
